...
const ProductCreateUpdateDetailPage = () => {
  ...
  const { data: addressQuery, isSuccess: getAddressSuccess } =
  ...
  const [
    deleteProduct,
    { data: deleteProductData, isLoading: deleteProductLoading },
  ] = useDeleteProductMutation();
  const imageUrl = Form.useWatch("imageUrl", formImageUrl);

  useEffect(() => {
    if (getUsersSuccess) {
      const { name, email, role, status, picture, ...rest } =
  ...
  return (
    <AdminLayout>
                  ...
                  >
                    {true ? (
                      <div className="ant-upload-drag-content">
                        <Spin
                          size="large"
                          indicator={<LogoCube loading size={28} />}
                        />
      ...
      <Drawer
        ...
        title={
          selectedAddress?._id ? `Địa chỉ · ${selectedAddress._id}` : "Địa chỉ"
        }
        footerStyle={{
          display: "flex",
            ...
            <Popconfirm
              title={
                ...
                >
                  ...
                  {selectedAddress?._id ? <b>{selectedAddress._id}</b> : ""}?
                </Typography.Paragraph>
              }
              placement="bottomRight"
              okText={<BsCheckLg />}
